...
func (producer *Producer) registerWithBroker(port uint16, topicID uint16) error {
	...
	err = WriteMessageRegisterToStream(stream_rw, PRODUCER_REGISTER, message.PRODUCER_REGISTER)
	if err != nil {
		return err
	}
...
func (producer *Producer) StartProducerServer(port uint16, topicID uint16) error {
	var err error
	...
	err = producer.registerWithBroker(port, topicID)
	if err != nil {
		return err
	}
